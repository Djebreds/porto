	...
	const items = $derived(
		stats
			? [
					...
					{
						icon: GitCommit,
						label: `${stats.contributionsCollection.totalCommitContributions} Commits (${currentYear})`
					},
					{ icon: GitPullRequest, label: `${stats.pullRequests.totalCount} Pull Requests` },
					{
						icon: CircleDot,
						label: `${stats.openIssues.totalCount + stats.closedIssues.totalCount} Issues`
					},
...
<div class="flex h-full flex-col transition duration-200 group-hover/bento:translate-x-2">
	<div class="mb-6 flex items-start justify-between gap-4">
		...
		<svg
			viewBox="0 0 24 24"
			class="h-12 w-12 shrink-0 opacity-20 sm:h-16 sm:w-16"
			fill="currentColor"
			aria-hidden="true"
		>
			<path
